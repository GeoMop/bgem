...
from bgem.src.bgem.external import undo
from bgem.src.bgem.polygons import aabb_lookup, decomp
...
class PolygonDecomposition:
    ...
    def _add_point_impl(self, point):
        ...
        TODO: make consistent system to check ide effects of decomp operations.
    ...
    def _add_line_for_points(self, a_pt, b_pt, omit=set(), deformability=1):
        ...
        TODO: intersecting two segments with very small angle we may add two
        ...
        for pt in candidate_pt:
            ...
            if dist < self.tolerance:
                ...
                omit_pt = omit | {pt}
                return self._add_line_for_points(a_pt, pt, omit=omit_pt) + self._add_line_for_points(pt, b_pt, omit=omit_pt)
