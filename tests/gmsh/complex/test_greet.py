import bgem.src.bgem.gmsh.gmsh_file as gmsh_file
import numpy as np
import pytest
import os
...
def create_cylinder(gmsh_occ, cyl_geom, stretch_factor=0.005):
    """
    Auxiliary function, creates elongated cylinder by given `stretch_factor`.
...
def test_empty_mesh():
    ...
    of the two tunnels intersection, that may lead to creating to overlapping surface elements on different
    surfaces.
    """
    ...
    # ...
    # Info    : Found volume 6
    # Info    : It. 0 - 0 nodes created - the worst tet radius 1.87415 (nodes removed 0 0)
