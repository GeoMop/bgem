...
import numpy as np
import collections
from bgem.src.bgem import gmsh
from bgem.src.bgem.gmsh import options as gmsh_options
from bgem.src.bgem.gmsh import field as gmsh_field
from bgem.src.bgem.stochastic import frac_plane as FP
from bgem.src.bgem.stochastic import frac_isec as FIC
...
def compute_intersections(fractures: fr_set.Fracture):
    ...
    for i in p:
        for j in p[i + 1:n_fr]:  # may be reduced to relevant adepts
            ...
            if possible_collision or frac_isec.have_collision:
                print(f"collision: {frac_isec.fracture_A.id}, {frac_isec.fracture_B.id}")
            assert not possible_collision or frac_isec.have_collision
