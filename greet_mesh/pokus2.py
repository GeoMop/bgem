...
def generate_mesh(geometry_dict):
    ...
    splits = []
    # # split_boxes = []
    splits.append(tunnel_split["start"].copy())
    ...
    #
    # tunnel_1_c.invalidate()
    # tunnel_1_x.invalidate()
    # tunnel_2_c.invalidate()
    # tunnel_2_x.invalidate()
    # tunnel_box_1.invalidate()
    # tunnel_box_2.invalidate()
    # side_y.invalidate()
    #
    # for o in frag:
    #     o.invalidate()
...
###########################################################################################
    ...
    # does not work
    # box_inner_cut = box_inner.cut(*tunnel_f)
    # box_inner_reg = frag[0].intersect(box_inner_cut)

    # try to cut - does not work
    # splits2 = [tunnel_split["end"].copy().translate([0, +10, 0])]
    # box_inner_cut = box_inner.cut(*tunnel_f)
    # # box_inner_reg = box_inner.cut(*tunnel_f)
    # box_inner_reg = gen.fragment(box_inner_cut, *splits2)[1]
    #
    # split box only - works
    # frag = gen.fragment(box_inner, *splits)
    # box_inner_reg = frag[0]

    # split box only - works
    splits2 = [tunnel_split["end"].copy().translate([0, -10, 0])]
    frag = gen.fragment(box_inner, *splits2)
    box_inner_reg = gen.group(frag[0])
    # box_inner_reg
#########################################################################################
    ...
    # b_tunnel = b_box_inner.select_by_intersect(tunnel_1_c)
    ...
    # tunnel_1_f.invalidate()
    # tunnel_1_c.invalidate()
    # tunnel_1_x.invalidate()
    # tunnel_2_f.invalidate()
    # tunnel_2_c.invalidate()
    # tunnel_2_x.invalidate()
    # tunnel_box_1.invalidate()
    # tunnel_box_2.invalidate()
...
def generate_mesh_cube(geometry_dict):
    ...
    frag = gen.fragment(box_inner.copy(), tunnel_1_c, tunnel_2_c, *splits)
    # frag = gen.fragment(box_inner.copy(), tunnel_1_c, *splits)
    ...
    box_inner_cut = box_inner.cut(*tunnel_f)
    # box_inner_reg = frag[0].cut(tunnel_box_1, tunnel_box_2)
    # box_inner_reg = frag[0].cut(*tunnel_f)
    box_inner_reg = frag[0].select_by_intersect(box_inner_cut)
    # box_inner_reg = frag[0].cut(*tunnel_f)
...
def generate_mesh_cut_tunnel(geometry_dict):
    ...
    tunnel_1.set_mesh_step(1.0)
    tunnel_2.set_mesh_step(1.0)
    #
    # # gen.synchronize()
    # mesh_all = [*box_all]
...
def generate_mesh_cylinder_discrete():
    # options.Geometry.Tolerance = 0.0001
    # options.Geometry.ToleranceBoolean = 0.001
    # options.Geometry.AutoCoherence = 2
    # options.Geometry.OCCFixSmallEdges = True
    ...
    gen = gmsh.GeometryOCC("cylinder_discrete_mesh")

    r = 2.5
    # axis = np.array([2,6,4])
    # axis = np.array([0, 20, 0])
    start = np.array([0, 75, 0])
    end = np.array([3, 17.180586073, 0])
    axis = end-start
    center = (start+end)/2
    cyl = gen.cylinder_discrete(r,axis,center=center, n_points=24)
    ...
    print("Generating mesh...[finished]")
    print("Writing mesh...")
    mesh_name = "cylinder_discrete_mesh.msh2"
    gen.write_mesh(mesh_name, gmsh.MeshFormat.msh2)
    print("Writing mesh...[finished]")
    # gen.show()
...
if __name__ == "__main__":
    ...
    # generate_mesh_cube(geometry_dict)
    generate_mesh_cut_tunnel(geometry_dict)

    # generate_mesh_cylinder_discrete()
    print("finished")